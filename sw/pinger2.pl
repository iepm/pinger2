...
#my $version="2.0.4, 2/5/2012 by LesCottrell";#Verify lynx returns something
...
sub log_it {
	...
	if (!(sysopen(PINGDATA, $ping_data_fn, (O_RDWR | O_EXCL | O_CREAT)))) {
		...
		print STDERR scalar(localtime()). " Can't open $ping_data_fn: $!\n";
...
if(!defined($config->{'PingV4Cmd'})) {
  print "Unable to get \$config->'{'PingV4Cmd} from pinger.xml\n";
}
&logger(scalar(localtime()). " $0(version=$version) "
      . "loaded pinger.xml config for "
